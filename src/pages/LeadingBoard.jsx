...
const kpiCards = [
  { label: "Total Leads",          value: "2,847",  change: "+12.5%", sub: "vs last month", positive: true  },
  { label: "Qualified Leads",      value: "1,423",  change: "+8.3%",  sub: "vs last month", positive: true  },
  { label: "Conversion Rate",      value: "23.5%",  change: "-2.1%",  sub: "vs last month", positive: false },
  { label: "Avg. Time to Convert", value: "14 days",change: "-3 days",sub: "vs last month", positive: true  },
];
...
const allLeads = [
  { name: "Dr. Sarah Chen",    specialty: "Oncology",      institutions: ["Mayo Clinic", "Stanford Health Care"],                          stage: "Opportunity",        score: 87, date: "Feb 12, 2025" },
  { name: "Dr. James Wilson",  specialty: "Cardiology",    institutions: ["Cleveland Clinic"],                                             stage: "Sales Qualified",    score: 74, date: "Feb 11, 2025" },
  { name: "Dr. Maria Garcia",  specialty: "Neurology",     institutions: ["Johns Hopkins Hospital", "Massachusetts General"],              stage: "Marketing Qualified", score: 68, date: "Feb 10, 2025" },
  { name: "Dr. Robert Kim",    specialty: "Endocrinology", institutions: ["Massachusetts General", "Mayo Clinic", "Boston Medical Center"],stage: "Lead (All)",          score: 52, date: "Feb 9, 2025"  },
  { name: "Dr. Emily Patel",   specialty: "Rheumatology",  institutions: ["Stanford Health Care"],                                         stage: "Opportunity",        score: 91, date: "Feb 8, 2025"  },
  { name: "Dr. Thomas Nguyen", specialty: "Pulmonology",   institutions: ["Mayo Clinic", "Cleveland Clinic"],                              stage: "Sales Qualified",    score: 79, date: "Feb 7, 2025"  },
];
      ...
      <main className="flex-1 overflow-auto">
        <div className="flex flex-col gap-6 p-8">
          ...
          <div className="flex flex-col gap-1">
            <h1 className="text-2xl font-medium leading-9 text-[#0a0a0a]">Lead board</h1>
            <p className="max-w-[700px] text-base leading-6 text-[#4a5565]">
              Monitor lead progression, funnel performance, and institutional engagement
            </p>
          </div>
          ...
          <div className="grid grid-cols-4 gap-4">
            {kpiCards.map((card) => (
              <div key={card.label} className="flex flex-col gap-2 rounded-[10px] border border-[#e5e7eb] bg-white p-6">
                <span className="text-sm font-medium text-[#6a7282]">{card.label}</span>
                <span className="text-2xl font-semibold text-[#0a0a0a]">{card.value}</span>
                <div className="flex items-center gap-1.5">
                  {card.positive ? <TrendUpSmallIcon /> : <TrendDownSmallIcon />}
                  <span className={`text-xs font-medium ${card.positive ? "text-[#16a34a]" : "text-[#dc2626]"}`}>
                    {card.change}
                  </span>
          ...
          <div className="grid grid-cols-5 gap-6">
            ...
            <div className="col-span-3 flex flex-col gap-5 rounded-[10px] border border-[#e5e7eb] bg-white p-6">
              <div className="flex items-center justify-between">
                <h2 className="text-base font-semibold text-[#0a0a0a]">Lead Funnel</h2>
              ...
              <div className="flex flex-col gap-3">
                {funnelStages.map((stage) => (
                  <div key={stage.name} className="flex items-center gap-4">
                    <span className="w-[140px] shrink-0 text-sm font-medium text-[#4a5565]">{stage.name}</span>
                    <div className="relative h-8 flex-1 rounded-md bg-[#f3f4f6]">
                      <div
                        className="flex h-full items-center rounded-md px-3"
                        style={{ width: `${Math.max(stage.pct, 12)}%`, backgroundColor: stage.color }}
                      >
                        <span className="text-xs font-semibold text-white">{stage.count.toLocaleString()}</span>
                      </div>
                    </div>
                    <span className="w-[52px] text-right text-sm font-medium text-[#6a7282]">{stage.pct}%</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="col-span-2 flex flex-col gap-4 rounded-[10px] border border-[#e5e7eb] bg-white p-6">
              <h2 className="text-base font-semibold text-[#0a0a0a]">Top Institutions</h2>
              <div className="flex flex-col divide-y divide-[#f3f4f6]">
                {topInstitutions.map((inst, i) => {
                  const tc = tierColors[inst.tier];
                  return (
                    <div key={inst.name} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="text-sm font-medium text-[#0a0a0a]">{inst.name}</span>
                          <span className="text-xs text-[#9ca3af]">Score: {inst.score}</span>
                        </div>
                      </div>
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${tc.bg} ${tc.text}`}>
                        Tier {inst.tier}
                      </span>
                    </div>
                  );
          ...
          <div className="flex flex-col rounded-[10px] border border-[#e5e7eb] bg-white">
            <div className="flex items-center justify-between border-b border-[#e5e7eb] px-6 py-4">
              <h2 className="text-base font-semibold text-[#0a0a0a]">New Leads</h2>
              <div className="flex items-center gap-3">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#e5e7eb] bg-[#f9fafb]">
                  {["Name", "Specialty", "Institution", "Stage", "Score", "Date Added"].map((h) => (
                    <th key={h} className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-[#6a7282]">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-[#f3f4f6]">
                {filteredLeads.map((lead) => (
                  <tr key={lead.name} className="transition-colors hover:bg-[#f9fafb]">
                    <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-[#0a0a0a]">{lead.name}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-[#4a5565]">{lead.specialty}</td>
                    <td className="px-6 py-4 text-sm text-[#4a5565]">{lead.institutions.join(", ")}</td>
                    <td className="whitespace-nowrap px-6 py-4">
                      <span className={`inline-block rounded-full px-2.5 py-0.5 text-xs font-medium ${stageBadge(lead.stage)}`}>
                        {lead.stage}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-[#0a0a0a]">{lead.score}</td>
                    <td className="whitespace-nowrap px-6 py-4 text-sm text-[#6a7282]">{lead.date}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <div className="flex items-center justify-between border-t border-[#e5e7eb] px-6 py-3">
              <span className="text-sm text-[#6a7282]">Showing 1–{filteredLeads.length} of 48 results</span>
